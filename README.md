...
process**, not Java. It reads like "Ghidra doesn't work here", and it is the thing this
...
* **The release zip does not contain the decompiler source** and has no `buildNatives`
  ...
  tag — only `Ghidra/Features/Decompiler/src/decompile/`, which is small.
* **The decompiler Makefile only knows x86.** `ARCH_TYPE` is `-m64` for `x86_64` and `-m32`
  ...
  on the command line overrides it — command-line variables win even against an assignment
  inside an `ifeq`.
...
* `GHIDRA_TAG` defaults to `Ghidra_<GHIDRA_VER>_build` — bump both together, since the
  native must match the release.
* The Dockerfile resolves the release zip from the GitHub release API, because the asset
  filename contains a build date (`ghidra_12.1.3_PUBLIC_20260817.zip`) that cannot be derived
  from the version. Set `GHIDRA_URL` to pin one exactly or to build offline.
* `build.sh` warns if the host is not arm64: the natives are built for the host, so the
  resulting image only runs there.

## CI

`.github/workflows/build.yml` builds on an **arm64 runner** (native, and free for public
repos — building the native under qemu on x86 would be an order of magnitude slower),
verifies that the decompiler really works by decompiling a small aarch64 binary, and pushes
`ghcr.io/kristofer84/ghidra-arm64` tagged with the version and `latest`.

Trigger it by pushing a `v<version>` tag, or manually with a version input.
